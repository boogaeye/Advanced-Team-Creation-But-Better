...
namespace AdvancedTeamCreation
{
    public class Translations : ITranslation
    {
        ...
        {
            { Team.ClassD, "You are a <color=orange>Class D</color>,:Find a <color=yellow>Keycard</color> and Escape!" },
            { Team.Scientists, "You are a <color=yellow>Scientist</color>,:Upgrade your <color=yellow>Keycard</color>:Get help and Escape!" },
            { Team.SCPs, "You are a <color=red>SCP</color>,:<color=red>Kill everyone...</color>" },
            { Team.ChaosInsurgency, "You are <color=green>The Insurgency</color>,:take <color=red>SCP</color> objects:Help <color=orange>Class D's</color> escape..." },
            { Team.FoundationForces, "You are the <color=blue>Mobile Task Force</color>,:take out <color=red>SCP Subjects</color>:Terminate <color=green>Hostile</color> Forces:Help <color=yellow>Scientists</color> escape..." },
        };

        public Dictionary<Team, string> TeamDisplayers { get; set; } = new Dictionary<Team, string>()
        {
            { Team.ClassD, "<color=orange>Class D Personal Escapee's</color>" },
            { Team.Scientists, "<color=yellow>Research Personal Escapee's</color>" },
            { Team.SCPs, "<color=red>SCP's</color>" },
            { Team.ChaosInsurgency, "<color=green>The Insurgency</color>" },
            { Team.FoundationForces, "<color=blue>Mobile Task Force</color> and <color=grey>Facility Forces</color>" },
        };

        public Dictionary<Team, string> TeamCassieSlaughter { get; set; } = new Dictionary<Team, string>()
        {
            { Team.ClassD, "pitch_0.5 .g4 .g4 .g4 pitch_1 all {Terminated} terminated by {Terminating} . allremaining awaitingrecontainment jam_1_5 {SCPLeft} scpsubjects" },
        };

        public Dictionary<Team, string> TeamCassieSlaughterSubtitles { get; set; } = new Dictionary<Team, string>()
        {
            { Team.ClassD, "all {Terminated} terminated by {Terminating} all remaining personal are advised to proceed with standard evacuation protocols until an MTF squad reaches your destination. awaiting recontainment of {SCPLeft} scp subjects." }
        };
    }
}